...
def news_view(request):
    url = 'https://newsapi.org/v2/everything?q=perbankan&apiKey=e5cac2437c82460babf1d584b37eb92c'
    try:
...
def report_user_view(request):
    ...
    # Batasi tampilan hanya ke data milik user
...
# -------------------------------
# Validasi, Komputasi, dan Penyimpanan IRK
# (dipisah menjadi fungsi-fungsi modular untuk memudahkan white box testing)
# -------------------------------

def check_completeness(responses):
    """Mengecek apakah seluruh 16 indikator sudah dijawab."""
    return responses.count() == 16


def check_range(scores):
    """Mengecek apakah seluruh skor hasil konversi berada pada rentang 1-5."""
    return all(1 <= s <= 5 for s in scores)


def validate_input(responses):
    """
    Validasi kelengkapan dan rentang skor jawaban responden.
    Mengembalikan (status, messages, scores).
    """
    status = 200
    messages = []
    ...
    complete = check_completeness(responses)
    if complete:
        scores = [response_mapping[r.response] for r in responses]
        in_range = check_range(scores)
        if in_range:
            status = 200
            messages.append("Data valid, lanjut ke perhitungan")
        else:
            status = 0
            messages.append("Nilai skor di luar rentang 1-5")
    else:
        status = 0
        messages.append("Terdapat butir yang belum terisi")

    return status, messages, scores


def load_fixed_weights():
    """Memuat bobot tetap (fixed model) hasil PCA untuk 16 indikator."""
    return [0.412, 0.302, 0.425, 0.087, 0.109, 0.056, 0.389, 0.106,
            0.454, 0.436, 0.051, 0.023, 0.056, 0.156, 0.068, 0.036]


def calculate_irk(scores):
    """
    Menghitung skor IRK mentah menggunakan bobot tetap hasil PCA.
    Mengembalikan (status, messages, irk).
    """
    status = 200
    messages = []
    irk = 0.0

    complete = len(scores) == 16
    if complete:
        weights = load_fixed_weights()
        if weights:
            weighted_sum = sum(w * s for w, s in zip(weights, scores))
            irk = (weighted_sum / sum(weights)) * 20
            messages.append(f"IRK terhitung: {irk:.2f}")
        else:
            status = 0
            messages.append("Bobot fixed model tidak ditemukan")
    else:
        status = 0
        messages.append("Data 16 indikator belum lengkap")

    return status, messages, irk
...
def check_data_valid(user, irk):
    """Mengecek validitas data sebelum disimpan ke basis data."""
    return user is not None and irk is not None and irk >= 0


def save_to_db(user, nama, pekerjaan, kecamatan_name, desa_name, irk, category):
    """
    Menyimpan hasil perhitungan IRK ke tabel base_irkresult.
    Mengembalikan (status, messages).
    """
    status = 200
    messages = []

    valid = check_data_valid(user, irk)
    if valid:
        try:
            ...
            status = 200
            messages.append("Data berhasil disimpan")
        except DatabaseError:
            status = 0
            messages.append("Gagal menyimpan, rollback dilakukan")
    else:
        status = 0
        messages.append("Data tidak valid untuk disimpan")
...
def generate_map(level="desa"):
    shp_path = os.path.join(settings.BASE_DIR, "mystaticfiles", "shp", "ADMINISTRASIDESA_AR_25K.shp")
...
def calculate_average_response(request):
    """
    Fungsi orkestrator: mengambil data responden, lalu memanggil
    validate_input() -> calculate_irk() -> categorize_irk() -> save_to_db()
    secara berurutan mengikuti alur ETL (Sub-bab 4.2.2).
    """
    user = request.user
    ...
    # Hitung IRK melalui fungsi-fungsi modular
    if not responses.exists():
        irk = 0.0
        category = "Belum Ada Data"
    else:
        val_status, val_messages, scores = validate_input(responses)
        if val_status != 200:
            return HttpResponse(val_messages[0])

        calc_status, calc_messages, irk = calculate_irk(scores)
        if calc_status != 200:
            return HttpResponse(calc_messages[0])

        category = categorize_irk(irk)

    save_status, save_messages = save_to_db(
        user, nama, pekerjaan, kecamatan_name, desa_name, irk, category
    )
